...
1. [ ] Foundations: API strategy and standards
   1. [ ] Confirm API base path `/api/v1` and content negotiation `application/json; charset=utf-8`.
   2. [ ] Ensure stateless auth (Authorization: Bearer <token>) for all `/api/v1/*`; do not use session AuthFilter (see Filters.php). Add an `ApiAuthFilter` for token validation. 
   3. [ ] CORS: Implement a CORS filter for SPA origins; handle OPTIONS preflight for `/api/v1/*`.
...
3. [ ] Authentication endpoints (stateless)
   1. [ ] POST /api/v1/auth/login: validate credentials (e.g., PIN), issue JWT/access token; 201 on success.
   2. [ ] POST /api/v1/auth/logout: optional server-side token revocation/blacklist.
   3. [ ] POST /api/v1/auth/agreement/accept: record acceptance (returns 204/200).
...
4. [ ] Users and access control
   1. [ ] GET /api/v1/users: list with pagination/filtering; projection fields.
   2. [ ] GET /api/v1/users/{id}
   3. [ ] POST /api/v1/users (admin only)
   4. [ ] PUT/PATCH /api/v1/users/{id}
   5. [ ] DELETE /api/v1/users/{id}
   6. [ ] GET /api/v1/users/{id}/roles
   7. [ ] PUT /api/v1/users/{id}/roles: replace roles (respect user_roles ON DELETE CASCADE)
   8. [ ] GET /api/v1/roles, GET /api/v1/capabilities
   9. [ ] GET /api/v1/roles/{id}/capabilities
   10. [ ] PUT /api/v1/roles/{id}/capabilities (role_capabilities ON DELETE CASCADE)

5. [ ] Companies and departments
   1. [ ] GET /api/v1/companies
   2. [ ] GET /api/v1/companies/{id}
   3. [ ] CRUD /api/v1/companies (consider preventing DELETE if cascading effects undesired)
   4. [ ] GET /api/v1/companies/{id}/departments
   5. [ ] Departments CRUD; note: departments.company_id ON DELETE CASCADE

6. [ ] Products and units
   1. [ ] GET /api/v1/products; GET /api/v1/products/{id}
   2. [ ] Units lookup: GET /api/v1/units
   3. [ ] CRUD products (FK: products.default_unit_id -> units)

7. [ ] Stations, tanks, reels, sensors (dispensing topology)
   1. [ ] Stations: GET /api/v1/stations, GET /api/v1/stations/{id}
   2. [ ] Reels: GET /api/v1/reels, GET /api/v1/reels/{id}
   3. [ ] Tanks: GET /api/v1/tanks, GET /api/v1/tanks/{id} (include ETag; aligns with existing TankApiController)
   4. [ ] Sensors: GET /api/v1/sensors, GET /api/v1/sensors/{id}
   5. [ ] Tank-Station visibility map: GET /api/v1/tank-station-map; CRUD respecting composite PK and CASCADE.
...
8. [ ] Devices and device types
   1. [ ] GET /api/v1/device-types
   2. [ ] GET /api/v1/devices, GET /api/v1/devices/{id}
...
10. [ ] Audit log
   1. [ ] GET /api/v1/audit-log with filters (user_id, event_id, station_id, device_id, tank_id, date range)
   2. [ ] Note: audit_log uses ON DELETE SET NULL for most FKs; ensure joins handle nulls.

11. [ ] App configuration and lookups
   1. [ ] GET /api/v1/config (public subset is_public=1)
   2. [ ] Admin config CRUD: POST/PUT/PATCH /api/v1/config/{config_key} (PK is string; useAutoIncrement=false)
   3. [ ] Lookups: GET /api/v1/languages, /time-zones, /units, /device-types, /system-events, /capabilities, /roles
...
15. [ ] Routing & filters
   1. [ ] Consolidate api/v1 route groups to avoid mixed filters; ensure CSRF is disabled for `/api/v1/*` and no session AuthFilter.
...
16. [ ] Implementation milestones (initial subset for SPA)
   ...
   2. [ ] Implement GET /api/v1/stations using StationModel.
   3. [ ] Implement GET /api/v1/reels using ReelModel.
   4. [ ] Implement GET /api/v1/products (add ProductModel) and GET /api/v1/units.
