...
class Pagination
{
	...
	function create_links()
	{
		...
		//Replacement of get instance from CI3
		$CIRequest = \CodeIgniter\Config\Services::request();
		$CIUri = \CodeIgniter\Config\Services::uri();
		try {
			if ($this->page_query_string === TRUE) {
				if ($CIRequest->getVar($this->query_string_segment) != $base_page) {
					$this->cur_page = $CIRequest->getVar($this->query_string_segment);

					// Prep the current page - no funny business!
					$this->cur_page = (int) $this->cur_page;
				}
			} else {
				if ($CIUri->getSegment($this->uri_segment) != $base_page) {
					$this->cur_page = $CIUri->getSegment($this->uri_segment);
		...
		if ($this->first_link !== FALSE and $this->cur_page > ($this->num_links + 1)) {
			...
			// $first_url = site_url('admin/pagination/wo_status/0');
